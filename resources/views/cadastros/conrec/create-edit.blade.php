...
<div class="container">
  ...
  @if(isset ($conrecEdit))
  @foreach ($conrecEdit as $conrecEdits)
  @endforeach
  {!! Form::model($conrecEdits, ['route' => ['conrec.update', $conrecEdits->Cod ], 'enctype' => 'multipart/form-data', 'method' => 'put']) !!}
  @else
  {!! Form::open(['route' => 'conrec.pesquisa', 'enctype' => 'multipart/form-data', 'class' => 'form']) !!}
  @endif
  ...
  {!! Form::open(['route' => 'conrec.store', 'enctype' => 'multipart/form-data', 'class' => 'form']) !!}
    ...
    <div class="form-row align-items-center">
      ...
      <div class="form-group col-sm-5">
        ...
        <select name="CLIENTE" class="form-control">
          ...
          <option value="{{$clientes->Cod}}">{{$clientes->nome}}</option>
          @endforeach
        </select>
      </div>
      <div class="form-group col-sm-1">
        <a style="margin-bottom: -28px;" href="{{route('cliente.create')}}" class="btn btn-success form-control col-12"><i class="fas fa-plus-square" style="font-size:22px;"></i></a>
      </div>
    </div>
    <div class="form-row align-items-center">
    ...
    <div class="form-row align-items-center">
      <div class="form-group col-sm-12">
        <table class="table table-sm">
          ...
          <tr>
            <td>{{$conrec->NUMERO}}</td>
            <td>{{$conrec->CLIENTE}} - {{$conrec->Nome}} </td>
            <td>{{$conrec->DATA}}</td>
            <td>{{$conrec->DTPAG}}</td>
            <td>{{$conrec->DTVENC}}</td>
            <td>{{$conrec->dsNomeMedico}}</td>
            <td>{{$conrec->PAGO}}</td>
            <td>{{$conrec->VALOR}}</td>
            <td>
              <a href="{{route('conrec.destroy', $conrec->NUMERO)}}" class="actions delete">
